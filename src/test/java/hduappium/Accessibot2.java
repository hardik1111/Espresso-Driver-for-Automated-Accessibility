...
public class Accessibot2 extends Baseclass15 {
	...
	private void collectAllElements() throws InterruptedException, IOException {
	...
	/**
	 * Retry mechanism for finding elements to handle transient failures due to dynamic UI changes.
	 */
	private List<WebElement> retryFindElements(By by) {
	    final int maxAttempts = 3;
	    for (int attempt = 1; attempt <= maxAttempts; attempt++) {
	        try {
	            return driver.findElements(by);
	        } catch (StaleElementReferenceException e) {
	            System.out.println("Attempt " + attempt + ": Failed to find elements, retrying...");
	...
	private void navigateToHomePage() throws InterruptedException {
    	...
    	Thread.sleep(10000);
    	driver.findElement(AppiumBy.accessibilityId("Product filter Favorites inactive")).click();
    	Thread.sleep(10000);
	}
